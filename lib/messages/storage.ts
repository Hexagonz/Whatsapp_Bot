...
export class Storager extends UserHandler {
    ...
    private async SaveMedia() {
        globalThis.CMD.on('storage|save <id, media>', 'save', async (res: WAConnection, data: Commands) => {
            ...
            if (
                ...
                if (!/(MB|KB|B)/i.test(size.symbol))
                    return this.Ra.sendTextWithMentions(from, IndFileGede(sender), [sender], mess)
                if (size.value >= 50.0 && size.symbol == 'MB')
                    return this.Ra.sendTextWithMentions(from, IndFileGede(sender), [sender], mess)
                const Path: string = './lib/storage/public/' + sender.replace(/@s.whatsapp.net/gi, '') + args[0]
                fs.readdir('./lib/storage/public/', async (err, call) => {
                    const total: number = call.filter((duh: string) =>
                        duh.startsWith(sender.replace(/@s.whatsapp.net/gi, ''))
                    ).length
                    ...
                    if (call.find((v) => v.startsWith(sender?.replace(/@s.whatsapp.net/gi, '') + args[0])))
                        return this.Ra.reply(from, IndIdDuplicate(), mess)
    ...
    private async getMedia() {
        globalThis.CMD.on('storage|get <id>', 'get', async (res: WAConnection, data: Commands) => {
            ...
            if (args[0] == undefined)
                return res.sendMessage(from, IndMasukkanId(), MessageType.text, {
                    quoted: mess
                })
            if (!sender) return
            fs.readdir('./lib/storage/public/', async (err, call) => {
                const data: string[] = call.filter((v) =>
                    v.startsWith(sender.replace(/@s.whatsapp.net/gi, '') + args[0])
                )
                if (data) {
