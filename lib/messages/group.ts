...
export class GroupData extends MusicHandling {
    ...
    protected AddGroup() {
        globalThis.CMD.on(
            ...
            async (res: WAConnection, data: Commands) => {
                ...
                if (!isOwner) return
                if (!isGroupMsg && !isGroupAdmins && !isGroupMsg && !isBotAdmins)
                    return isGroupMsg
                        ? !isBotAdmins
                            ? isGroupAdmins
                                ? this.Ra.reply(from, BotGaAdmin(), mess)
                                : undefined
                            : undefined
                        : this.Ra.reply(from, BukanDalamGroup(), mess)
                if (mentioned && mentioned[0] !== undefined) {
                    if (groupMember !== null ? groupMember?.map((value) => value.jid).includes(mentioned[0]) : true)
                        return this.Ra.reply(from, UserDalamGroup(), mess)
    ...
    protected kickGroup() {
        globalThis.CMD.on(
            ...
            async (res: WAConnection, data: Commands) => {
                ...
                if (
                    groupMember !== null
                        ? !groupMember?.map((value) => value.jid).includes(mentioned !== undefined ? mentioned[0] : '')
                        : true
                )
                    return this.Ra.reply(from, UserGadaDalamGroup(), mess)
                if (mentioned && mentioned[0] !== undefined) {
                    if (mentioned[0] == ownerGroup) return this.Ra.reply(from, isOwnerGroupNokick(), mess)
                    if (
                        groupMember !== null
                            ? groupMember
                                  ?.filter((value) => value.isAdmin == true)
                                  .map((value) => value.jid)
                                  .includes(mentioned[0])
                            : (false && ownerGroup == sender) || isOwner
                    ) {
    ...
    protected linkgroup() {
        globalThis.CMD.on(
            'admingc|linkgc',
            ['linkgroup', 'linkgc', 'linkgrup'],
            async (res: WAConnection, data: Commands) => {
                const { isBotAdmins, from, isGroupAdmins, groupMetadata, mess, isGroupMsg, isOwner } = data
                if (!isGroupMsg) return this.Ra.reply(from, BukanDalamGroup(), mess)
                if (!isBotAdmins) return this.Ra.reply(from, BotGaAdmin(), mess)
                let LinkGc: string | undefined = (await Object.keys(
                    groupMetadata !== null ? groupMetadata : []
                ).includes('desc'))
                    ? groupMetadata?.desc
                    : ''
                if (isGroupAdmins || isOwner) {
                    ...
                    this.Ra.reply(
                        from,
                        IndLinkGroup(groupMetadata, 'https://chat.whatsapp.com/' + (await res.groupInviteCode(from))),
                        mess
                    )
    ...
    protected OpenCloseGc() {
        globalThis.CMD.on(
            ...
            async (res: WAConnection, data: Commands) => {
                ...
                if (!isBotAdmins) return this.Ra.reply(from, BotGaAdmin(), mess)
                if (!isOwner && !isGroupAdmins) return
                let BukaTutup: boolean | null = /^(open|buka)/i.test(args[0])
                    ? false
                    : /^(close|tutup)/i.test(args[0])
                    ? true
                    : null
            ...
            async (res: WAConnection, data: Commands) => {
                ...
                if (mentioned && mentioned[0] == undefined && isOwner) {
                    ...
                    !groupMetadata?.participants
                        .filter((value) => value.isAdmin == true)
                        .map((value) => value.jid)
                        .includes(mentioned !== undefined ? mentioned[0] : '')
                ) {
                    await res.groupMakeAdmin(from, [mentioned !== undefined ? mentioned[0] : ''])
                    await this.Ra.sendTextWithMentions(
                        from,
                        PromoteSuccess(mentioned !== undefined ? mentioned[0] : ''),
                        mentioned || [],
                        mess
                    )
                } else {
                    await this.Ra.sendTextWithMentions(
                        from,
                        PromoteDiaAdmin(mentioned !== undefined ? mentioned[0] : ''),
                        [mentioned !== undefined ? mentioned[0] : ''],
                        mess
                    )
                }
            }
    ...
    protected Demote() {
        globalThis.CMD.on('admingc|demote <tag/reply>', ['demote'], async (res: WAConnection, data: Commands) => {
            const { isBotAdmins, from, isGroupAdmins, mess, isGroupMsg, mentioned, isOwner, sender, groupMetadata } =
                data
            if (!isGroupMsg) return this.Ra.reply(from, BukanDalamGroup(), mess)
            if (!isBotAdmins) return this.Ra.reply(from, BotGaAdmin(), mess)
            if (!isOwner || !isGroupAdmins) return
            if (mentioned && mentioned[0] == undefined) {
                ...
                await this.Ra.sendTextWithMentions(
                    from,
                    DemoteSuccess(mentioned !== undefined ? mentioned[0] : ''),
                    mentioned || [],
                    mess
                )
            } else {
                await this.Ra.sendTextWithMentions(
                    from,
                    DemoteBukanAdmin(mentioned !== undefined ? mentioned[0] : ''),
                    [mentioned !== undefined ? mentioned[0] : ''],
                    mess
                )
            }
        ...
        globalThis.CMD.on(
            ...
            async (res: WAConnection, data: Commands) => {
                ...
                if (isGambar || isQuotedImage) {
                    ...
                    const Input: string = await res.downloadAndSaveMediaMessage(
                        media,
                        './lib/storage/temp/' + RandomName(22)
                    )
                    await Toimg(Input)
    ...
    protected setNameGc() {
        globalThis.CMD.on(
            'admingc|setnamegc <text>',
            ['setnamagc', 'setnamegc', 'setnamegrup', 'setnamagroup'],
            async (res: WAConnection, data: Commands) => {
                const { isBotAdmins, from, isGroupAdmins, mess, isGroupMsg, args, isOwner } = data
                if (!isGroupMsg) return this.Ra.reply(from, BukanDalamGroup(), mess)
                if (!isBotAdmins) return this.Ra.reply(from, BotGaAdmin(), mess)
    ...
    protected setBioGc() {
        globalThis.CMD.on(
            'admingc|setbiogc <text>',
            ['setdesk', 'setdeskgc', 'setdesc'],
            async (res: WAConnection, data: Commands) => {
                const { isBotAdmins, from, isGroupAdmins, mess, isGroupMsg, args, isOwner } = data
                if (!isGroupMsg) return this.Ra.reply(from, BukanDalamGroup(), mess)
                if (!isBotAdmins) return this.Ra.reply(from, BotGaAdmin(), mess)
    ...
    protected Hidetag() {
        globalThis.CMD.on(
            'admingc|hidetag <img,vid,stick,txt>',
            ['hidetag', 'hidden'],
            async (res: WAConnection, data: Commands) => {
                const {
                    isBotAdmins,
                ...
                const Members: string[] | undefined = groupMember
                    ?.filter((value: WAGroupParticipant) => value.isAdmin == false)
                    .map((value: WAGroupParticipant) => value.jid)
                if ((isOwner && Members) || (isBotAdmins && isGroupAdmins && Members)) {
                    if ((isVideo && media) || (isQuotedVideo && media)) {
                        await res.sendMessage(from, await res.downloadMediaMessage(media), MessageType.video, {
                            contextInfo: { mentionedJid: Members },
                            caption: args.join(' ')
                        })
                    } else if ((isGambar && media) || (isQuotedImage && media)) {
                        const Data: Buffer | any = await res.downloadMediaMessage(media)
                        await res.sendMessage(from, Data, MessageType.image, {
                            thumbnail: Data,
                            contextInfo: { mentionedJid: Members },
                            caption: args.join('')
                        })
                    } else if (isQuotedSticker && media) {
                        await res.sendMessage(from, await res.downloadMediaMessage(media), MessageType.sticker, {
                            contextInfo: { mentionedJid: Members }
                        })
                    } else if (!isQuotedDokumen || !isMedia) {
                        mentioned
                            ? await res.sendMessage(from, args.join(' '), MessageType.extendedText, {
