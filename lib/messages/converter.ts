...
export class Converter {
    ...
    protected async Sticker(): Promise<void> {
        globalThis.CMD.on(
            'converter|sticker <img,vid,sticker>',
            ['sticker', 's', 'stiker', 'stickergif', 'stikergif', 'sgif'],
            async (res: WAConnection, data: Commands) => {
                ...
                if (CheckSticker(from, FileSha || 'undefined', Stick)) {
                    ...
                    const Respon:
                        ...
                        | undefined = Hasil.find((value) => value.filesha == FileSha)
                    await this.Ra.sendTextWithMentions(
                        from,
                        StickerDuplicate(Respon?.sender || '', Respon?.id || 0),
                        [Respon?.sender || ''],
                        mess
                    )
                ...
                await CreateSticker(input, Wm)
                    .then(async (result: string | Error) => {
                        try {
                            if (typeof result !== 'string') return
                            let ress: proto.WebMessageInfo = await res.sendMessage(
                                from,
                    ...
                    .catch(async () => {
                        await CreateSticker(input, Wm)
                            ...
                            .catch(async () => {
                                await createStickerV2(input, Wm)
                                    .then(async (Result) => {
                                        try {
                                            if (typeof Result !== 'string') return
                                            let ress: proto.WebMessageInfo = await res.sendMessage(
                                    ...
                                    .catch(async () => {
                                        await createStickerV3(input, Wm)
                                            .then(async (Res) => {
                                                try {
                                                    if (typeof Res !== 'string') return
                                                    let ress: proto.WebMessageInfo = await res.sendMessage(
    ...
    protected async ToVideo() {
        globalThis.CMD.on(
            ...
            async (res: WAConnection, data: Commands) => {
                ...
                const input: string = await res.downloadAndSaveMediaMessage(
                    media,
                    './lib/storage/temp/' + RandomName(23) + sender?.replace('@s.whatsapp.net', '')
                )
                await ToVideo(input)
                    .then(async (value: { status: number; data: string } | any) => {
    ...
    protected async toMP3() {
        globalThis.CMD.on(
            ...
            async (res: WAConnection, data: Commands) => {
                ...
                const input: string = await res.downloadAndSaveMediaMessage(
                    media,
                    './lib/storage/temp/' + RandomName(24)
                )
                await Tomp3(input)
                    .then(async (result: string | Error) => {
                        if (typeof result !== 'string') return
    ...
    protected async toImg() {
        globalThis.CMD.on(
            ...
            async (res: WAConnection, data: Commands) => {
                ...
                const result: string = await Toimg(
                    await res.downloadAndSaveMediaMessage(data.media, './lib/storage/temp/' + RandomName(27))
                )
    ...
    protected async toCUTE() {
        globalThis.CMD.on(
            ...
            async (res: WAConnection, data: Commands) => {
                ...
                const input: string = await res.downloadAndSaveMediaMessage(
                    media,
                    './lib/storage/temp/' + RandomName(28)
                )
