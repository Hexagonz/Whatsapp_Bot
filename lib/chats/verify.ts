...
export class Verify {
	...
	private async Verify () {
		globalThis.CMD.on("user|verify", ["verify"], async (res: WAConnection, data: Commands) => {
			...
			const value: { otp: string, hit: number, status: boolean, open: boolean } | undefined = register.get(sender || "")
			...
			if (value && Number(value.hit) === 3) {
				...
				register.set(sender || "", Format)
				const hasil: { otp: string, hit: number, status: boolean, open: boolean } | undefined = register.get(sender || "")
