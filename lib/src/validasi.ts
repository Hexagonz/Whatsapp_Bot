...
export class Validation {
    ...
    public validator(chats: WAMessage | undefined): Validasi {
        ...
        if (
            type === 'extendedTextMessage' && message?.message?.extendedTextMessage
        ...
        const typeQuoted: string =
            type == 'extendedTextMessage' && message?.message?.extendedTextMessage
                ? Object.keys(
                      message.message.extendedTextMessage.contextInfo
                          ? message.message.extendedTextMessage.contextInfo.quotedMessage
                              ? message.message.extendedTextMessage.contextInfo.quotedMessage
                              : { mentionedText: 'RA BOT' }
                          : { thumbnailMessage: 'I`am Ra' }
                  )[0]
                : type
        const quotedMsg: proto.IContextInfo | null | undefined = message?.message?.extendedTextMessage
            ? message.message.extendedTextMessage.contextInfo
            ...
            : null
        const bodyQuoted: string | null | undefined =
            typeQuoted === 'conversation'
                ...
                : ''
        const CommandButton: string | null | undefined = message?.message?.buttonsResponseMessage
            ? message.message.buttonsResponseMessage.selectedDisplayText
            : ''
        const getIdButton: string | null | undefined = message?.message?.buttonsResponseMessage
            ? message.message.buttonsResponseMessage.selectedButtonId
            : ''
        const body: string | null | undefined = message?.message?.conversation
        ...
        const media: WAMessage | null =
            message?.message?.imageMessage || message?.message?.videoMessage
                ...
                ? JSON.parse(JSON.stringify(message).replace('quotedM', 'm')).message?.extendedTextMessage?.contextInfo
                : null
        const mentioned: string[] | undefined[] =
            ...
            message.message.extendedTextMessage.contextInfo.mentionedJid.length > 0
                ...
                ? [message.message.extendedTextMessage.contextInfo.participant]
                : []
        const sender: string | null | undefined = chats?.key.fromMe
            ? this.client.user.jid
            : isGroupMsg
            ? chats?.participant
            : chats?.key.remoteJid
        const FileSha: string | null | undefined = message?.message?.imageMessage
            ? message.message.imageMessage.fileSha256?.toString()
            : message?.message?.videoMessage
            ? message.message.videoMessage.fileSha256?.toString()
            : message?.message?.stickerMessage
            ? message.message.stickerMessage.fileSha256?.toString()
            : quotedMsg
            ? quotedMsg.quotedMessage?.imageMessage
                ? quotedMsg.quotedMessage.imageMessage.fileSha256?.toString()
                : quotedMsg.quotedMessage?.videoMessage
                ? quotedMsg.quotedMessage.videoMessage.fileSha256?.toString()
                : quotedMsg.quotedMessage?.stickerMessage
                ? quotedMsg.quotedMessage.stickerMessage.fileSha256?.toString()
                : null
            : null
        const Filesize: number | Long | undefined | null = media
            ? media.message
                ? media.message.audioMessage
                    ? media.message.audioMessage.fileLength
                    : media.message.imageMessage
                    ? media.message.imageMessage.fileLength
                    : media.message.videoMessage
                    ? media.message.videoMessage.fileLength
                    : media.message.documentMessage
                    ? media.message.documentMessage.fileLength
                    : 0
                : null
            : null
